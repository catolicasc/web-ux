...
$(document).ready(function () {
    ...
    const items = [
            ...
            shipments: [
                {
                    ...
                    quantity: 1,
                    isDelayed: false,
                    details: "Entregue em 21/01/2025",
                    progressStep: 7,
                    history: [
                        {
                            "code": "P_DELIVERY",
                        ...
                        },
                        {
                            ...
                            "noteList": [
                                ...
                                {
                                    "code": "PREVISION_STOCK",
                                    "description": "Previsão de estoque",
                                    "content": "Adicionar data, validar com gabriel"
                                },
                            ],
                            "dates": {
                                "previsto": "13/01/2025",
        ...
        },
        {
            ...
            shipments: [
                {
                    id: "REM-015", // Mudar a logica disso, esta quebrando se a remessa esta igual, ou garantir que remessas sao unicas
                    name: "#10452204",
                    status: "completed",
                    statusText: "Entregue",
                    date: "21/01/2025",
    ...
    function populateTimeline(shipmentId) {
        ...
        // Encontra a remessa correspondente
        ...
        // Organiza os passos com base no histórico da remessa
        const stepsWithStatus = [];

        // Mapeia o histórico para os passos da timeline
        ...
        $.each(reversedSteps, function (index, step) {
            ...
            if (step.status === 0) {
                ...
                // Passo em andamento - mostra previsto e revisado
                const previsto = step.historyEntry && step.historyEntry.dates ?
                    step.historyEntry.dates.previsto : "Pendente";
            ...
            let historyHTML = '';
            const historyId = `${step.code}`;

            // Ícone de histórico para todos, mas desabilitado para não concluídos
            let historyIconHTML = '';
            if (step.status === 2 && step.historyEntry) {
                historyIconHTML = `
                            <i class="ion-chevron-up history-toggle-icon" id="history-toggle-${historyId}"></i>
                        `;
                let notesHTML = '';
                if (step.historyEntry.noteList && step.historyEntry.noteList.length > 0) {
                    console.log(step.historyEntry)
                    $.each(step.historyEntry.noteList, function (_, note) {
                        // if (note.code == "INVOICE") {
                        //     notesHTML += `<p class="history-text"><strong>${note.description}</strong>
                        //                         <a href="${invoiceUrlStart}${note.content}${invoiceUrlEnd}" target="_blank">${note.content}</a>
                        //                     </p>`;
                        // } else if (note.code == "TRACKING_URL") {
                        //     notesHTML += `<p class="history-text"><strong>${note.description}</strong>
                        //                         <a href="${note.content}" target="_blank">Acesse o site aqui <i class="search-outline ion-search"></ion-icon></a>
                        //                     </p>`;
                        // }
                        // else {
                            notesHTML += `<p class="history-text"><strong>${note.description}</strong> ${note.content}</p>`;
                        // }
                    });
                }
        ...
        // Adiciona eventos de clique aos ícones de histórico
        addHistoryToggleEvents();
    }

    // Função para adicionar eventos de clique aos ícones de histórico
